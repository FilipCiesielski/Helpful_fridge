...
class FridgeBox extends Component {
    ...
    render() {
        ...
        if (this.state.response === null) {
            ...
            responseResult = this.state.response.meals.map((meal) => <li key={meal.idMeal}
                                                                         onClick={() => this.handleOnClick(meal.strMeal)}>{meal.strMeal}</li>)
            recipeResult =
                <>
                    <div className={"recipe_box"}>
                        <div className={"ingredient_list"}>
                            <ul className={"list"}>
                                {(this.state.ingredients === null) ? null : this.state.ingredients.map((ingr, index) =>
                                    <li key={index}>{ingr}</li>)}
                            </ul>
                        </div>
                        <div className={"measure_list"}>
                            <ul className={"list"}>
                                {(this.state.measure === null) ? null : this.state.measure.map((mea, index) => <li
                                    key={index}>{mea}</li>)}
                            </ul>
                        </div>
                    </div>
                    <div className={"method"}>
                        {(this.state.method === null) ? null : this.state.method.map((met, index) => <p
                            key={index}>{met}</p>)}
                    </div>
                </>
        }
